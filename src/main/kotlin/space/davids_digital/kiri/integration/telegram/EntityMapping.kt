...
fun Message.toModel(): TelegramMessage {
    return TelegramMessage(
        ...
        paidMedia = paidMedia()?.toModel(),
        photo = photo()?.map { it.toModel() },
        ...
        voice = voice()?.toModel(),
        caption = caption(),
        captionEntities = captionEntities()?.map { it.toModel() },
        ...
        venue = venue()?.toModel(),
        location = location()?.toModel(),
        newChatMembers = newChatMembers()?.map { it.id() },
        leftChatMember = leftChatMember()?.id(),
        newChatTitle = newChatTitle(),
        newChatPhoto = newChatPhoto()?.map { it.toModel() },
